...
class GooglePOI(object):
    def __init__(self,api_key,itemy,loc,radius):
    ...
    def urls(self):
        ...
        for pages in range(0,25):
            url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' + self.loc +'&radius='+self.radius+'&offset=25&page=' + str(pages) + '&type=' + self.itemy + '&key=' + self.api_key
            urls.append(url)
        return urls
...
if __name__ == '__main__':
    ...
    for item in n:
        ...
        for loc_to_use in locs_to_use:
            ...
            print loc_to_use
            lat1 = loc_to_use.split(',')[0];
            lon1 = loc_to_use.split(',')[1];
            lat2 = loc_to_use.split(',')[2];
            lon2 = loc_to_use.split(',')[3];
            ...
            par = GaoDePOI(api_key,cate,loc_to_use,radius)            #请修改这里的参数
            a = par.baidu_search()
